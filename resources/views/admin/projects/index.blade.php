<x-app-layout>
    ...
    <div class="py-10 bg-gray-50 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-md sm:rounded-2xl p-6 border border-gray-100 space-y-4 transition-all duration-300 ease-in-out">

                @forelse ($projects as $project)
                    <div class="item-card flex flex-col md:flex-row justify-between items-start md:items-center bg-gray-50 hover:bg-gray-100 transition rounded-xl p-5 border border-gray-200 shadow-sm hover:-translate-y-1 hover:shadow-lg duration-300 ease-in-out">
                                    ...
                                    @if ($project->assignments && $project->assignments->count())
                                        <div class="flex flex-wrap gap-2 mt-2">
                                            @foreach ($project->assignments->take(6) as $a)
                                                <div class="px-3 py-1 rounded-full bg-indigo-100 text-indigo-800 text-xs font-medium">
                                                    {{ $a->user->name }}
                                                </div>
                                            @endforeach
